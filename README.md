...
- **대화형**: 문장을 쓰면 규칙 기반 파서가 "기간 + 사람 조건 + 대상/지표"로 해석해 **라벨-값 요약 카드**로 보여줌(확정=체크+텍스트, 미확정=절제된 셀렉트). LLM 없음. 두 모드는 상태 공유. `@` 입력 시 **이벤트 자동완성 드롭다운**(↑↓·Enter·Esc, 선택 시 라벨 삽입→파서가 이벤트로 해석).
