...
- **대화형(v6)**: 자연어 질문 → GAS 웹앱 프록시가 Gemini 2.5 Flash로 **BigQuery SQL을 직접 생성**(규칙파서로 불가했던 CTR·비율 등도 처리). 프록시가 실측 카탈로그(선택 프로퍼티 1개, 트림)를 grounding으로 주입해 이벤트명·파라미터 환각을 막고, 출력가드(읽기전용·`_TABLE_SUFFIX`·허용 데이터셋·카탈로그 존재 여부)+자가수정 1회로 검증한다. 프록시 실패/미배포 시 기존 규칙 파서 해석 칩으로 자동 폴백. 두 모드는 상태 공유. `@` 입력 시 **이벤트 자동완성 드롭다운**(↑↓·Enter·Esc).
