...
## 코어
- **레이아웃**: 상단 카테고리 바(대화형/셀렉형 + 찾을 데이터 고방/U사장님 + 이벤트 사전) · 좌 작성 · 우 큰 미리보기+SQL.
...
- **대화형**: 문장을 쓰면 규칙 기반 파서가 "기간 + 사람 조건 + 대상/지표"로 해석해 칩으로 보여줌. 애매하면 노란 되물음 칩, 확정되면 SQL 생성. LLM 없음. 두 모드는 상태 공유.
- **기간**: 프리셋 pill(7/14/30) + "직접 선택"(누를 때만 날짜 범위 펼침).
- **이벤트 사전**: 모달 이벤트 행 옆 ⓘ → 설명 + 파라미터 표(타입·설명, 전역은 '공통' 태그). 어느 파라미터가 어느 이벤트에 있는지(상속 결과 포함) 확인용.
