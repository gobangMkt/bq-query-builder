...
## 코어
- **레이아웃(3열 워크벤치)**: 좌 사이드바(프로퍼티·대화형/셀렉형·섹션 내비) · 중앙 스크롤(입력) · 우 미리보기+SQL. 각 열 독립 스크롤. 셀렉형은 사이드바 섹션 내비로 단계 이동(스크롤 스파이).
- **입력 2모드**:
  - **대화형**: 문장을 쓰면 규칙 기반 파서가 "기간 + 사람 조건 + 대상/지표"로 해석해 칩으로 보여줌. 애매하면 노란 되물음 칩(후보 선택), 확정되면 SQL 생성. LLM 없음.
  - **셀렉형**: 기간 → 이벤트 → 사람 조건 → 지표·열(집계/상세) → 필터. 두 모드는 상태를 공유.
- **기간**: 프리셋 pill(7/14/30) + "직접 선택"(누를 때만 날짜 범위 펼침).
- **이벤트 사전**: 이벤트 행 옆 ⓘ → 설명 + 파라미터 표(타입·설명, 전역은 '공통' 태그). 어느 파라미터가 어느 이벤트에 있는지(상속 결과 포함) 확인용.
...
- **스택**: Vite + vanilla TypeScript (런타임 의존성 0), vitest 84 + Python Playwright E2E 19
