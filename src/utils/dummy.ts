// 구조 미리보기용 표본값 생성. 실 BQ 조회 없이, 컬럼별로 실데이터 형태의 대표값(sample-values.ts)을
// 우선 채우고 없으면 타입 기반 예시값으로 폴백한다.
...
import { sampleValuesFor } from '../data/sample-values';
...
/** 배열을 원하는 길이만큼 순환 반복해 채운다. */
function cycle(values: string[], count: number): string[] {
  return Array.from({ length: count }, (_, i) => values[i % values.length]);
}

/**
 * 컬럼별 표본값 3행. 컬럼명에 매칭되는 실데이터형 표본값(sample-values.ts)을 우선 쓰고,
 * 없으면 타입 기반 예시값으로 폴백한다. 숫자 폴백은 열마다 오프셋으로 값을 살짝 달리한다.
 */
export function computePreviewRows(columns: PreviewColumn[]): string[][] {
  ...
  const columnValues = columns.map((col) => {
    const sample = sampleValuesFor(col.name);
    if (sample) return cycle(sample, PREVIEW_ROW_COUNT);
